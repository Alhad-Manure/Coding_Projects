...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  // ...
  std::queue<TreeNode*> nonExpand;
  nonExpand.push(rootNodePtr);
  while(!nonExpand.empty())
  {
    
    std::vector<TreeNode*> curNodes = nonExpand.front()->childrenPtrs;
    results.push_back(nonExpand.front()->data);
    nonExpand.pop();
