...
ISR(INT1_vect)
{
 ...
 wrdata('D');
 _delay_ms(2000);
 //Above lines for Simulation testing only
...
float ADC_result(char i)
{
 	...
 	t = ADCL;								 //Lower 8 bits of ADC result
 	u = ADCH;								 //Upper 2 bits of ADC result
...
void measure_bat_voltage()
{
 ...
 battery_voltage = 1 * 0.0048 * battery_voltage;	// For simulation testing
...
void loop()
{
 ...
 char t,u; 
 measure_bat_voltage();
 //sprintf(buf,"%2.2f",cur_avg_volt);
 t = ((cur_avg_volt / 10) + 0x30);
 u = ((cur_avg_volt % 10) + 0x30);
 wrcmd(0x01);
 wrcmd(0x80);
